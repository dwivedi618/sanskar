...
export class ManageAcademicYearComponent implements OnInit {
  ...
  private add(formValues){
    this.academicYearApiService.addMasterFeeCategory(formValues).subscribe((result) => {
  ...
  private update(formValues){
    this.academicYearApiService.updateMasterFeeCategory(formValues).subscribe((result) => {
